...
| P4 | **Partial** | Plugin contracts, typecheck/tests, Paseo 0.8 migration, installed-plugin health, scoped disable/reenable, and live core subprocess boundary pass. Live installed-plugin RPC dispatch, typed input validation, and contributed-RPC allowlist enforcement now pass through the supported `plugin.rpc.invoke` path. Functional task flow over RPC is blocked because the running daemon lacks `PASEO_KNOWLEDGE_*`; plugin UI panel rendering and ordinary-path runtime compatibility remain open. |
...
| P7 | **Open** | The 20-query evaluation and five bounded installed Python/core tasks pass, including cross-task approved reuse; measured warm search/context p95 targets pass; Paseo 0.8 migration/current plugin health and scoped disable/reenable pass. Live plugin RPC dispatch now passes. Integrated acceptance still requires functional RPC task flow, plugin UI evidence, ordinary-path runtime compatibility, and a fresh criterion-level audit. |
...
- Git delivery and branch cleanup evidence: [`docs/evidence/knowledge-vault-git-delivery.md`](../evidence/knowledge-vault-git-delivery.md); full local/origin inventory: [`docs/evidence/knowledge-vault-branch-inventory-20260919.md`](../evidence/knowledge-vault-branch-inventory-20260919.md).
...
- Live RPC and client connectivity diagnosis: [`docs/evidence/knowledge-vault-live-rpc-connectivity-20260919.md`](../evidence/knowledge-vault-live-rpc-connectivity-20260919.md); the daemon WebSocket endpoint `/ws` is healthy and five plugin RPCs were dispatched live through the supported client. Two blockers are newly diagnosed: the running daemon carries a stale launchd job definition without `PASEO_KNOWLEDGE_*`, and `features.webUi.enabled` is not in the daemon's reloadable path set. Both need one approved daemon restart.
